...
Provide `build/icon.ico` and `build/icon.icns` before packaging. Builds are
intentionally unsigned and do not identify a publisher.
...
Reaction Clipboard is intentionally offline-only. The renderer CSP denies
network connections, and the main process blocks remote HTTP, HTTPS, WebSocket,
navigation, permission, and new-window attempts. Development permits loopback
Vite assets only.
...
clipboard APIs do not reliably paste GIFs as animated attachments, so GIF card
clicks copy a still image with explicit feedback. Discord clipboard behavior
still requires manual platform validation.
